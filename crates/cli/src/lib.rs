...
pub fn index_workspace(path: &str) -> Result<()> {
    ...
    let quiet = std::env::var("RUST_RAG_QUIET").is_ok();
    ...
    let mut saved_state = state::IndexState::load(&store_path)?;
    ...
    let embedding_time = embed_start.elapsed().as_secs();
    saved_state.set_embedding_time(embedding_time);
...
pub fn ask(query: &str, workspace_root: Option<&str>) -> Result<()> {
    ...
    let ws = if let Some(p) = workspace_root {
        std::path::PathBuf::from(p)
    } else {
        std::env::current_dir().expect("Failed to get current directory")
    };
    if let Ok(cfg) = rust_rag_core::config::Config::load(&ws) {
        if cfg.auto_reindex {
            let _ = index_workspace(ws.to_str().unwrap());
        }
    }
    let result = run_ask_impl(query, workspace_root, OutputMode::Text);
    // Update LLM request count in state
    if result.is_ok() {
        if let Ok(mut state) = state::IndexState::load(&ws.join(".rustrag")) {
            state.increment_llm_requests();
            let _ = state.save(&ws.join(".rustrag"));
        }
    }
    result?;
    Ok(())
...
pub async fn ask_stream(query: &str, workspace_root: Option<&str>) -> Result<()> {
    ...
    let result = run_ask_stream_impl(query, workspace_root).await;
    // Update LLM request count in state
    if result.is_ok() {
        if let Ok(mut state) = state::IndexState::load(&ws.join(".rustrag")) {
            state.increment_llm_requests();
...
pub fn ask_json(query: &str, workspace_root: Option<&str>) -> Result<()> {
    ...
    let result = run_ask_impl(query, workspace_root, OutputMode::Json);
    // Update LLM request count in state
    if result.is_ok() {
        if let Ok(mut state) = state::IndexState::load(&ws.join(".rustrag")) {
            state.increment_llm_requests();
...
pub async fn ask_stream_json(query: &str, workspace_root: Option<&str>) -> Result<()> {
    ...
    let result = _ask_stream_json_internal(query, workspace_root).await;
    // Update LLM request count in state
    if result.is_ok() {
        if let Ok(mut state) = state::IndexState::load(&ws.join(".rustrag")) {
            state.increment_llm_requests();
...
pub fn show_stats(workspace_path: Option<&str>, json_output: bool) -> Result<()> {
    ...
    let state_path = store_path.join("index_state.json");
    let state = state::IndexState::load(&state_path).unwrap_or_default();
    let embedding_time = state.embedding_time_seconds;
    let llm_requests = state.llm_requests;
    ...
    if json_output {
        ...
        println!("  Embedding time (s): {}", embedding_time);
        println!("  LLM requests: {}", llm_requests);
